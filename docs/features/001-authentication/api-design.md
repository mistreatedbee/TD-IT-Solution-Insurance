...
- **Versioning:** `/v1/` on every path below, per `05-development-standards.md`'s `/api/v1/...` convention — this document uses `/v1/...` matching backend-approach.md's own stated grouping table verbatim; **flag to `devops-engineer`/whoever owns the gateway config:** confirm at deployment whether the platform-wide prefix is `/api/v1` or `/v1` before Stage 9 — backend-approach.md and `05-development-standards.md` are not textually identical on this point (`/v1/...` vs `/api/v1/...`) and neither Stage 5 nor Stage 6 caught it. This document uses `/v1/...` (matching Stage 5's own endpoint table) and flags the discrepancy rather than silently picking one without noting it.
