...
**Status:** Draft — screen-by-screen mockup specification, ready for `frontend-engineer`/`mobile-engineer` estimation input and `product-manager` sign-off. Three screens (§4.9 compound lockout, §4.10 lost-MFA-device at login, §4.4 step 9 recovery codes) are explicitly marked **pending mechanism confirmation** per `ux-researcher`'s handoff notes (ux-research.md §5.3) — happy-path layout is designed now; copy/flow may still change.
...
### Screen C — Recovery Codes *(pending confirmation — see flag below)*
- **[NEW: Recovery-code display — cybersecurity-architect + design-system-manager review pending]**. `ux-research.md` §1.4 step 9 assumes this exists as the account's only self-service path back in if the authenticator is lost, but §5.3 explicitly flags it as **not yet confirmed in scope** by `authentication-engineer`/`cybersecurity-architect`.
- **Design position taken here, per this task's explicit platform-owner ruling:** the compound-lockout case (password + MFA both lost) is resolved via **support-assisted manual reset**, not self-service recovery codes (see §4.9). This changes the *stakes* of this screen but does not, by itself, confirm or deny whether a single-factor-loss case (MFA device lost, password still known — `ux-research.md` §1.5 step 7's "Can't access your authenticator?" link) is handled by recovery codes or also routes to support. That narrower question remains open for `authentication-engineer`/`cybersecurity-architect`.
- **Two variants specified, pending that resolution — `frontend-engineer` should build against whichever is confirmed at Stage 5/6, not guess:**
  - **Variant 1 (recovery codes in scope):** `CardHeader title="Save your recovery codes"`, `CardBody` shows the [NEW: Recovery-code display] component — a set of one-time codes, copy/download action, explicit checkbox gate: "I've saved these codes somewhere safe" — must be checked before `Button variant="primary" fullWidth` — "Done" — enables. Framing copy: "If you ever lose access to your authenticator app, these codes are the only way back into your account. Store them somewhere secure — a password manager, not a screenshot on the same phone your authenticator lives on."
  - **Variant 2 (recovery codes descoped, single-factor MFA loss also routes to support):** this screen is **skipped entirely** — Screen B's success routes directly to dashboard home, and the "Can't access your authenticator?" link in §4.5 Screen B instead routes to the same support-escalation pattern as §4.9.
- This document does not pick between the two — it is flagged to `authentication-engineer`/`cybersecurity-architect` per `ux-research.md`'s own instruction, and `frontend-engineer` should not build Screen C until that's resolved.
...
- Destination depends on the §4.4 Screen C resolution: routes either to recovery-code entry (Variant 1) or directly to the support-escalation screen (Variant 2, same pattern as §4.9).
...
- Recovery codes (if in scope, §4.4 Screen C Variant 1) render as selectable/copyable text, never an image.
...
- **`authentication-engineer` / `cybersecurity-architect`:** §4.4 Screen C (recovery codes) scope decision is still open — this document specifies both variants and cannot finalize Screen C's copy/presence until that lands. This is narrower than the compound-lockout question (which this task's brief has now resolved via §4.9) — it's specifically about single-factor MFA-device loss with password still intact.
...
- [ ] **Sign-off obtained from product-manager (or delegate) before Development handoff.** Not yet obtained — this document is the artifact submitted for that sign-off, along with the open flags in §7 (particularly the §4.4 Screen C recovery-code scope decision, which materially affects two other screens' final copy).

**Net status:** Eight (nine, counting the compound-lockout flow called out separately in the task brief) screens/flows specified end-to-end using existing components wherever possible. Two checklist items are intentionally left unchecked and disclosed above (ux-researcher validation pass, product-manager sign-off) rather than self-certified, consistent with this role's charter that final sign-off is not unilateral. The compound-lockout screen (§4.9) is fully resolved per the platform-owner's ruling and is no longer an open placeholder; the narrower single-factor-MFA-loss recovery-code question (§4.4 Screen C) remains genuinely open pending `authentication-engineer`/`cybersecurity-architect` input and is flagged, not guessed at.
