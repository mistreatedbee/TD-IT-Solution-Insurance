...
Date opened: 2026-08-24 · Last updated: 2026-08-28 (Cartrack API-signal update — see §2 Candidate C; Standing Decision Rule reaffirmed — see §9)
...
- **Update (research pass, 2026-08-28):** Cartrack publishes a public developer portal at `developer.cartrack.com` with a documented Fleet API (vehicle location, trip data, driver information) and at least one confirmed third-party B2B integration precedent (a Mercedes-Benz Fleet Partner API integration, per public Mercedes-Benz Connectivity Services material) — this is a materially stronger G3 signal than the original 2026-08-24 pass assumed ("third-party API access needs direct commercial engagement to confirm" understated what's already public). **This narrows, but does not close, the open question below** — a documented fleet-telematics API existing publicly is not the same as (a) that API being licensable to a competing insurance-recovery platform rather than only OEM/fleet partners, or (b) Cartrack's monitoring center actioning a dispatch request that originates from our platform rather than their own alerting.
- **Open questions (revised):** whether Cartrack's public Fleet API is available for wholesale/insurer-platform licensing (not just OEM-partner-tier deals like Mercedes-Benz) at commercially reasonable B2B terms, and whether their Emergency Control Centre would action a recovery dispatch triggered externally by our platform rather than only by their own device alerts — both now the single biggest open item on this candidate, narrower than before but still unresolved without direct commercial engagement. This is exactly the fact the Standing Decision Rule (§9) turns on.
...
## 9. Standing Decision Rule (pre-authorized fallback — reaffirmed 2026-08-28)

Per `docs/organization/sprint-plan-release-gate-a.md` ("Standing Decision Rule — GPS hardware vendor default"): if Cartrack and Tracker do not commit to a documented third-party API on commercially reasonable B2B terms by **2026-10-05**, the OEM route (Digital Matter / Teltonika — Category 1, Candidates A/B above) becomes the **default selection automatically**. `integration-architect` does not require a fresh sign-off cycle to conclude that outcome once the deadline passes without commitment — it is pre-authorized, not a decision requiring new approval. This does not change the 2026-10-05 decision-due date, and does not authorize any change to `tracking-profile.ts`'s vendor-agnostic shape ahead of ratification (§0, §6 above still bind regardless of which path this resolves to). Recorded here so this scorecard is self-contained on the rule the sprint plan already established, rather than requiring a cross-document lookup at decision time.
