...
Status: **Ratified** — `cto`, 2026-08-24 (§17), also standing in for `solution-architect`, per `.cursor/rules/00-house-rules.mdc`. Ratification adopts the ADR-level precedent and is **conditional** on the four items at §17.3 — none of which reopens the decision. **The two concurrences at §15 and §16 remain outstanding and are conditions on this ratification, not discharged by it.** Concurrence still required before this closes: `compliance-specialist` (lawful basis, retention, and the data-class question at SDL-7/SDL-8 — a review is reported running in parallel this session and is **not** assumed here) and `security-engineer` (implementation-fidelity conditions on SDL-3, SDL-6, SDL-10). **The security-architecture requirements SDL-1 … SDL-12 below are binding now** under `cybersecurity-architect`'s standing authority over trust-boundary design (`06-security-standards.md` §Governance — the same authority ADR-0006's status line invokes). What was awaiting ratification — and what §17 ratifies — is the ADR-level *packaging*: the platform-wide precedent this sets for **every future stream of telemetry asserted by a customer-controlled client**, of which self-device location is merely the first.
Date: 2026-08-14
...
- **Ratification is outstanding.** This ADR is *Proposed*. SDL-1 … SDL-12 bind design work under this role's standing authority, but the ADR-level precedent is `cto`'s to ratify.
  - **[Superseded on 2026-08-24 by §17 — recorded, not rewritten.]** Ratification is no longer outstanding: `cto` ratified the ADR-level precedent on 2026-08-24, conditionally (§17.3). **Every other bullet in §14 stands** — ratification of the precedent is still not a Stage 8 sign-off, not a compliance ruling, not `product-manager` sign-off, not an API/schema approval, and not an instruction to add any client location capability. One factual update to the API bullet, recorded rather than rewritten: a Stage 7 draft contract now exists at [`008/api-design.md`](../../features/008-self-device-gps-tracking/api-design.md) (`gps-integration-engineer`, 2026-08-14, v1.0.0, Draft). Its existence approves nothing — it is a design artifact awaiting Stage 8, and it is one of the inputs that gate will review against SDL-1 … SDL-12. See §17.4.
...
## 17. `cto` ratification — **Ratified**, conditionally, with the two concurrences still open

**Approving authority:** `cto` (final decision authority for this platform; no separate CEO role exists) · **Effective date:** 2026-08-24 · **Filed by:** `solution-architect`, on `cto`'s direction of 2026-08-24 that ADR-0008 and ADR-0009, ratified in principle, have their ratification sections filed within one week. **Also acting for `solution-architect`** ratification where the two roles' authority overlaps, per `.cursor/rules/00-house-rules.mdc`, which names `cto` the fallback owner for that stage — the same basis on which ADR-0006 §16 was signed.

**Verdict: ratified.** The ruling at §0 — that self-device location reporting is a genuinely new trust boundary, that it gets its own ADR, and that ADR-0006 is neither reopened nor extended — is adopted as platform precedent. So are SDL-1 … SDL-12 in the ADR-level form in which they were filed, which they already held as binding design requirements under `cybersecurity-architect`'s standing authority.

**17.1 — Why this was ratified as an ADR rather than demoted.** §6 offered demotion to `docs/features/008-self-device-gps-tracking/security-architecture.md` if this classification was disagreed with. It is not. Two things make ADR-level correct, and the second is the stronger:

- **SDL-1 and SDL-2 are the reversal-expensive half.** The endpoint is cheap to unwind; a store of coordinates whose origin is no longer determinable is not, and neither is a claims or dispatch pathway that has already consumed a self-asserted signal.
- **The precedent reaches past Feature 008 and past location entirely.** This is the platform's first ruling on customer-asserted telemetry as a distinct provenance class. A self-declared asset condition, a customer-reported odometer reading, a self-reported theft time, any future non-hardware signal — all of them inherit SDL-1 and SDL-2 by name, and none of the roles who will build them would think to search a Feature 008 folder for the rule. This is exactly the reasoning behind ADR-0006's R-5 and it applies here with more force, not less.

**17.2 — What is *not* being re-decided.** No technical content is revisited. §10's "what does not change" stands in full: ADR-0006 unamended, ADR-0002 unamended, ADR-0001 unamended, no retention period set, no lawful-basis determination made, no hardware-vendor implication beyond SDL-1's discriminator. §8's seven residual risks are ratified as *accepted with their stated bounds* — in particular **RR-2** (an attacker inheriting an already-opted-in account's live position feed is not fully mitigated) is accepted knowingly, on the record, and is not treated as closed by anything in this section.

### 17.3 Conditions on this ratification

Ratified now because none of these reopens the decision, and holding them as conditions is cheaper than deferring the precedent while a feature that has not been approved waits for it:

1. **The two concurrences at §15 and §16 stay open and stay blocking.** Neither `security-engineer` (implementation fidelity on SDL-3, SDL-6, SDL-10) nor `compliance-specialist` (lawful basis, retention, and the SDL-7/SDL-8 data-class question) has filed a concurrence in this document. A `compliance-specialist` review does exist at [`008/compliance-review.md`](../../features/008-self-device-gps-tracking/compliance-review.md) and is not legal sign-off by its own terms; it is **not** read here as discharging §16, and SDL-8 still has no retention number to enforce. This ratification covers the **ADR-level packaging and the precedent**; it does not stand in for either role, and §13's two unchecked boxes stay unchecked. A negative `compliance-specialist` determination on lawful basis ends Feature 008 regardless of anything ratified here.
2. **SD-FU-01 fires now.** Its trigger was "on ratification," and ratification has happened: `cybersecurity-architect` files the normative line in `06-security-standards.md` §"Data protection & privacy" — telemetry asserted by a customer-controlled client is a distinct provenance class, labelled as such by the server at write time, and may not alone drive a real-world, third-party, or financial consequence. This is the FU-A5 pattern, and it is what makes the rule bind roles who read neither this ADR nor Feature 008's folder. It is not contingent on Feature 008 being approved.
3. **SDL-2 may not be relaxed by a product decision.** Recorded as a standing `cto`-level constraint, because it is the requirement most likely to be leaned on later: any proposal that would let a self-asserted coordinate dispatch a partner, inform a claim, alter pricing or eligibility, or be disclosed to a partner organisation as an instruction reopens this ADR, fires SDL-12's attestation requirement, and arrives at Stage 8 with a corroboration design or not at all.
4. **SD-FU-05 is severed from Feature 008 and is not allowed to wait on it.** The partner-organisation surface (`/v1/security/cases*`) writing no audit trail and carrying no AUD-9 purpose reference is a live gap in an already-built surface, found while ruling on a feature that does not exist. It is owned by `backend-architect` + `cybersecurity-architect` and is tracked independently of whether Feature 008 is ever built — if Feature 008 is cancelled, SD-FU-05 survives it. SD-FU-04 (`recovery_cases.lastLocation` carrying no provenance fields, RR-5) is likewise `database-architect`'s and binds the first writer of that field whatever feature that writer belongs to.

### 17.4 What I am not ratifying

- **Not authorization to build Feature 008.** §14 is the operative statement and it is unchanged apart from the superseded ratification bullet. Ratifying the precedent is not a green light for the feature, and no one may cite this section as one.
- **Not a Stage 8 sign-off.** That gate has not run for Feature 008. SDL-1 … SDL-12 are inputs to it, not a substitute for it, and the Stage 7 draft contract that now exists is subject to it rather than exempt from it.
- **Not a compliance or lawful-basis ruling**, and not a retention period (SDL-8 sets enforcement, not the number).
- **Not `product-manager` sign-off** on whether the feature is built at all.
