...
8. **ADR-0008 condition 1 — "applied, never verified" language.** Do not treat item 1.4 (ratification filed) or 2.8 (check exists) as closing this condition. It stays open until `devops-engineer` explicitly confirms the catalog-verification check runs in the live deploy path. This is a documentation-honesty risk of the same shape as INC-001's root cause (see Incident Log root-cause note) — track it as a discrete item, not folded into 1.4/2.8's general "done" status. **Correction, `devops-engineer`, 2026-09-10:** the narrow deploy-path question is now confirmed by code review — `render.yaml`'s `startCommand: npm start` runs `backend/src/index.ts`, which unconditionally calls `verifyMongoCatalog()` at `index.ts:78–94` (confirmed line range) immediately after Mongo bootstrap, on every deploy, no flag or env gate around it. Not independently confirmed against live Render deploy logs this pass — no Render MCP/log access was available in this session; that leg of confirmation is still outstanding. Separately unresolved, and **not decided here**: the check is deliberately non-fatal (log-only `console.log`/`console.error`, no non-zero exit, no CI/`render.yaml` gating step per condition 1 point 2) and nothing currently reads that log — whether that satisfies "verified" under condition 1, or whether a real drift finding should be louder than a log line, remains a design call for `devops-engineer`+`database-architect`/ADR-0008's owners, not resolved by this correction. Item 2.8 and this item stay open pending both the live-log confirmation and that design call; do not mark either "closed" from this note alone.
