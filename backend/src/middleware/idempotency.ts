 ...
 * mfa/enroll/verify, session/logout-all). Missing header -> 400
 * `IDEMPOTENCY_KEY_REQUIRED`. Same key + same body -> cached response
 * replayed verbatim. Same key + different body -> 409
...
export function requireIdempotencyKey(endpoint: string, repo: IdempotencyRepo) {
  return async function idempotencyMiddleware(req: Request, res: Response, next: NextFunction): Promise<void> {
    ...
    try {
      const existing = await repo.find(endpoint, key);
      if (existing) {
