...
export interface VerificationCentreItem {
  ...
  tone: 'neutral' | 'gold' | 'emerald' | 'warning' | 'danger';
  href?: string;
  meta?: string;
}
...
function toneForState(state: VerificationItemState): VerificationCentreItem['tone'] {
  switch (state) {
    case 'complete':
      return 'emerald';
    case 'in_review':
...
export function deriveVerificationCentreItems(input: {
  ...
  items.push({
    ...
    href: emailState === 'complete' ? undefined : '/login',
    meta: input.email,
  });
  ...
  for (const step of input.completionChecklist) {
    ...
    items.push({
      ...
      href: step.done ? undefined : '/dashboard/profile',
    });
  }
...
export function partitionVerificationItems(items: VerificationCentreItem[]) {
  const pending = items.filter((item) => item.state !== 'complete');
